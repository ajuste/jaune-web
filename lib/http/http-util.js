...
HttpUtil.prototype.generateEtag = function(stat) {
  ...
  handleResponseOnAuth = function* (opts) {
    let   dataAvailable = opts.result && opts.result.data != null;
    const clearData     = true === opts.clearData && dataAvailable;
    let   data          = dataAvailable ? opts.result.data : undefined;

    if (true === opts.sendNotFoundOnNoData && !data) {
      if (clearData) {
        data = undefined;
      }
  ...
  handleResponseAsDefault = function* (opts) {
    opts = _.defaults(opts || {}, { result : { data : null } });
  ...
  handleServiceError = function* (opts) {
    ...
    if (opts.dontFinishResponse !== true) {
      this.endWithCode(opts.response,
        isArgumentError ?
          _enums.HttpCode.BadRequest :
          _enums.HttpCode.InternalServerError,
          isArgumentError ? opts.err.message :
          null);
    }
  },*//*
