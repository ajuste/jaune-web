...
const sendData = function* (opts) {
  ...
  const isGrantedResult = this.jaune.engine().Security.IsGrantedResult;
...
const sendError = function* (err) {
  ...
  const isCodedError    = err instanceof engine.Error.UnhandledError;
  const util            = engine.Http.Util;
  const errorManager    = engine.Error.Manager;

  const logger          = this.logger;
  const loggerArgs      = this.loggerArgs;
  let   code            = isArgumentError ? _httpCode.BadRequest : 0;

  if (!code) {
    code = isCodedError && err.code ? err.code : _httpCode.InternalServerError
  }
  ...
  util.endWithCode(this, code, isCodedError ? err.message : null);
};
