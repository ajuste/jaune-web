 ...
 */
'use strict';
var GenericPage404, GenericPage500, PagesConfigSection, bind, chunk, defaultsDeep, extend, getDefaultLocalization, getGenericPageSettings, has, isNumber, isObject, lodash, render, resolveLocalization, resolvePageData, send, sendError, sendNotFound, splitIntoChunks;
...
getDefaultLocalization = function(page, settings, localeManager) {
  var key, localization, results, setting, value;
  localization = settings.localization;
  if (!(page["default"] && has(settings, 'localization.defaults'))) {
    return;
  }
  results = [];
  for (key in page.defaults) {
    if (localization.defaults[key] == null) {
      continue;
    }
...
resolveLocalization = function(page, settings, data) {
  ...
  if (keys != null) {
    clientLoc.push((function() {
      var results;
      results = [];
      for (key in pageLocalization.keys) {
        results.push({
          key: key,
          value: localeManager.getStringResource(page.localization.keys[key] + '1', true)
        });
      }
      return results;
    })());
