 ...
 */
'use strict';
var AppName, ConfigErrorSection, ConfigHttpSection, ConfigInitSection, ConfigLocaleSection, EnvType, EnvTypeDev, I18n, KoaServer, evaluateName, extend, handleError, http, https, koa, lodash, register;
...
KoaServer = (function() {
  function KoaServer(env, engine1) {
    this.env = env;
    this.engine = engine1;
  ...
  KoaServer.prototype.setupWeb = function() {
    var appName, args, cert, context, enabled, engine, html, httpServer, key, opts, port, ref, ref1;
    if (((ref = this.httpSettings) != null ? ref.web : void 0) != null) {
      ref1 = this.httpSettings.web, enabled = ref1.enabled, port = ref1.port, html = ref1.html, https = ref1.https;
    }
    opts = {};
    if (!enabled) {
      return;
    ...
    httpServer = https ? https : http;
    port = parseInt(port != null ? port : 3000, 10);
    if (html != null) {
      this.app.use(evaluateName(engine, args, context, {
        app: this.app
      }));
    }
    if (https != null) {
      ...
      return httpServer.createServer(opts, this.app.callback()).listen(port, function() {
        return console.log(appName + " web server started");
      });
    } else {
      return httpServer.createServer(this.app.callback()).listen(port, function() {
        return console.log(appName + " web server started");
      });
    }
  };
  ...
  KoaServer.prototype.setupErrorHandling = function() {
    ...
    return this.app.use(function*(next) {
      ...
      try {
        ...
        return (yield handleError(this, error));
      }
    });
  };
