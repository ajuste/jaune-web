...
const _bundles     = require("../bundles");
...
const KoaServer = function(env, engine) {
  ...
  this.bundleSettings = env.getEnvProperty(CONFIG_BUNDLES_SECTION);
...
KoaServer.prototype.setupWeb = function() {
  ...
  const appName = this.env.getEnvProperty(APP_NAME);
...
KoaServer.prototype.setupI18n = function() {
  ...
  this.app.use(function* (next) {
    var locale = this.getLocaleFromQuery() || this.getLocaleFromCookie() || this.getLocaleFromHeader() || localeManager.getDefaultCountry();
    if (locale) {
...
/**
 * @function Set up bundles
 */
KoaServer.prototype.setupBundles = function() {

  if (!_has(this, "bundleSettings")) return;
  if (!this.bundleSettings.enabled) return;

  _bundles(this.engine);
};
