...
KoaServer = (function() {
  ...
  KoaServer.httpSettings = KoaServer.env.getEnvProperty(ConfigHttpSection);

  KoaServer.initSettings = KoaServer.env.getEnvProperty(ConfigInitSection);

  KoaServer.localeSettings = KoaServer.env.getEnvProperty(ConfigLocaleSection);

  KoaServer.errorSettings = KoaServer.env.getEnvProperty(ConfigErrorSection);
  ...
  KoaServer.prototype.setupMiddlewares = function() {
    ...
    if (((ref = this.initSettings) != null ? (ref1 = ref.init) != null ? ref1.middlewares : void 0 : void 0) == null) {
      return;
    }
    return this.initSettings.init.middlewares(this.app, this.engine);
  };
  ...
  KoaServer.prototype.setupBody = function() {
    var koaBody, multipart, parse, ref, settings, strict, uploadPath;
    if ((typeof httpSettings !== "undefined" && httpSettings !== null ? httpSettings.body : void 0) != null) {
      ref = this.httpSettings.body, parse = ref.parse, multipart = ref.multipart, strict = ref.strict, uploadPath = ref.uploadPath;
    }
    if (!parse) {
      return;
    }
    ...
    return this.app.use((function(_this) {
      return function*(next) {
        ...
        try {
          ...
          err = error;
          if ((customErrorHandling != null) && ((yield customErrorHandling(err, _this, _this.session)))) {
            return;
          }
          (yield handleError(_this, err));
        }
...
handleError = function(ctx, err, self) {
  var stack;
