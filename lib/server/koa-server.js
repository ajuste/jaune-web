...
const _register   = require("../context").Register;
...
KoaServer.prototype.setupSession = function() {
  ...
  this.app.use(_reflection.callByName(settings.store, settings.storeArgs, settings.store.context, {app : this.app }));
};
...
KoaServer.prototype.setupStatic = function() {
  ...
  const args = {
    root: this.httpSettings.jaune.http.static.path,
    maxAge: httpSettings.jaune.http.static.maxAge
  };
...
KoaServer.prototype.setupMiddlewares = function() {
  ...
  this.app.use(_register(this.app, this.engine));
...
KoaServer.prototype.setupWeb = function() {
  ...
  const port = parseInt(this.configuration.web.port || 3000, 10);
  const html = this.httpSettings.web.html;

  if (html) {
    this.app.use(_reflection.callByName(html.engine, html.args, html.context, {app : this.app }));
  }
  http.createServer(this.app.callback()).listen(port, function(){
    console.log(this.env.appName + " server started");
  });
};
...
KoaServer.prototype.setupI18n = function() {
  ...
  const locale    = this.engine.Locale.Manager;

  if (_isObject(this.configuration.i18n)) {
    koaLocale(this.app);
    this.app.use(function* (next) {
      const locale = this.getLocaleFromQuery() || this.getLocaleFromCookie() || this.getLocaleFromHeader() || locale.getDefaultCountry()
      if (locale) {
        this.session.locale = yield locale.setLocale(locale);
...
KoaServer.prototype.setupRouting = function() {
  if (!_has(this, "initSettings.init.setupRoutes")) return;

  this.app.use(require("koa-routing")(this.app));
  this.initSettings.init.setupRoutes(this.app);
};
...
KoaServer.prototype.initDaemons = function() {
  if (!_has(this, "initSettings.init.initDeamons")) return;
  this.initSettings.init.initDeamons();
};
